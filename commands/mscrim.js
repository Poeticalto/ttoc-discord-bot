...
exports.run = async (client, message, args, level) => {
    ...
    if (message.channel.parent && leagueList.indexOf(message.channel.parent.name) > -1 && channelName.indexOf("general") === -1) {
        ...
        scrimCheck = client.scrimList.getScrimPlayer.get(message.channel.id+"-m");
        if (!scrimCheck) {
            // if team isn't on scrimlist, add team
            scrimCheck = {
                "id": message.channel.id+"-m",
                "name": teamName,
                "nametype": message.channel.parent.name.charAt(0).toLowerCase() + message.channel.parent.name.slice(1)
            };
            // write team data to db
            client.scrimList.setScrimPlayer.run(scrimCheck);
            await message.channel.send(teamName + " minors added to scrim list!").catch(console.error);
        }
        else {
            // otherwise, remove the team from the scrimlist db
            client.scrimList.deleteScrimPlayer.run(message.channel.id+"-m");
            await message.channel.send(teamName + " minors removed from scrim list!").catch(console.error);
        }
...
exports.help = {
    ...
    description: "Adds a minors team or a player to the scrim list depending on the channel",
    usage: "mscrim"
};