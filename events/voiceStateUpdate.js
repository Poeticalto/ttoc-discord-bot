...
{
    var removeCheck = guild.roles.find(role => role.name === abbrProcess);
    if (memberEdit.roles.has(removeCheck.id))
    {
        memberEdit.removeRole(removeCheck).catch(console.error);
    }
}